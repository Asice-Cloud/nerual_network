...
void Net::printWeights() {
    std::cout << "Weights from Input Layer to Hidden Layer:" << std::endl;
    for (size_t i = 0; i < Config::INNODE; ++i) {
        for (size_t j = 0; j < Config::HIDENODE; ++j){
            std::cout << "Input Node " << i << " to Hidden Node " << j << ": " << inputLayer[i]->weight[j] << std::endl;
        }
    }

    std::cout << "\nWeights from Hidden Layer to Output Layer:" << std::endl;
    for (size_t j = 0; j < Config::HIDENODE; ++j) {
        for (size_t k = 0; k < Config::OUTNODE; ++k) {
            std::cout << "Hidden Node " << j << " to Output Node " << k << ": " << hiddenLayer[j]->weight[k] << std::endl;
        }
    }
}